...
<?php if($jabatan == 0 || ($unit == '002')): ?>
    ...
    <div id="modal-buat-catatan-keu" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title mt-0 h-catatan"></h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
        ...
        <?php if($unit == '002' || $jabatan == 0): ?>
            ...
            $('#tb-detail-biaya').on('click', '.btn-buat_catatan_keu', function(){
                ...
                if(data.catatan_disetujui == ''){
                    $('#modal-buat-catatan-keu h5.h-catatan').text('Buat Catatan Realisasi')
                } else {
                    $('#modal-buat-catatan-keu h5.h-catatan').text('Ubah Catatan Realisasi')
                }

                $('#modal-buat-catatan-keu input[name="id"]').val(data.id)
                $('#modal-buat-catatan-keu textarea[name="catatan"]').val(data.catatan_disetujui)
