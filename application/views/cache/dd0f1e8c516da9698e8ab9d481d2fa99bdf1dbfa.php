        ...
        <div class="card-box mt-2">
            <div class="row mb-3">                    
                ...
                <div class="col-md-4 col-sm-4 col-xs-12">
                    <form action="<?php echo e(base_url('app/hibah/pencairan')); ?>" method="GET" accept-charset="utf-8" autocomplete="off">                        
                ...
                <table id="tb_data_hibah" class="table table-striped table-bordered dt-responsive nowrap">
                    <thead class="bg-purple text-white">
                        <tr>
                            ...
                            <th class="text-center" style="vertical-align: middle;">Anggaran Digunakan</th>
                        ...
                        ?>
                            <?php $__currentLoopData = $data['data']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <tr>
                                    ...
                                    <th style="vertical-align: middle;"><?php echo e($row['nama_karyawan']); ?></th>
                                    ...
                                    <td class="text-center" style="vertical-align: middle;">
                                        <?php
                                            ...
                                            AND b.disetujui = 'Y'", $row['kode_uraian']))->row();									
                                            echo rupiah($row['total_agr'] + ($getSumIn->saldo_masuk - $getSumOut->saldo_keluar));
                                        ?>
                                    </td>
                                    <td class="text-center" style="vertical-align: middle;">
                                        <?php
									        /*$getSum = $CI->db->query("SELECT SUM(a.agr) digunakan FROM ig_tbl_actbud a WHERE a.id_uraian = '$row[id]' AND a.status_act = 'send' AND a.status != 'cancel'")->row();
									        echo rupiah($row['total_agr'] - $getSum->digunakan)*/
                                            $getSum = $CI->db->query(sprintf("SELECT SUM(a.agr) digunakan FROM 
                                            ig_tbl_actbud a WHERE a.id_uraian = '%u' 
                                            AND a.status_act = 'send' AND (a.status != 'cancel')", $row['id']))->row();
                                            $getSumIn = $CI->db->query(sprintf("SELECT SUM(b.nominal) saldo_masuk FROM 
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.user', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\xampp1\htdocs\hibah_upj\application\views/users/hibah/v_data_pencairan.blade.php ENDPATH**/ ?>