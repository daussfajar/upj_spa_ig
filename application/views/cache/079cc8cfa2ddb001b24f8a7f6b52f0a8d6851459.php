                        ...
                        ?>
                            <?php $__currentLoopData = $data['data']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center" style="vertical-align: middle;">
                                        <a href="<?php echo e(base_url('app/sponsorship/pencairan/v_detail/'.encrypt($row['id_hibah']).'/actbud/'.encrypt($row['id_actbud']))); ?>" class="badge bg-info">Lihat</a>
                                    </td>
                                    <td class="text-center" style="vertical-align: middle;">
                                        <?php if($row['status_actbud'] == 'approved'): ?>
                                            <a href="javascript:void(0)" onclick="window.open('<?php echo e(base_url('app/sponsorship/pencairan/v_detail/'.encrypt($row['id_hibah']).'/actbud/'.encrypt($row['id_actbud']).'/cetak_form_actbud?pdf=true')); ?>', 'MsgWindow', 'width=800,height=800')" class="btn btn-primary btn-xs text-white"><i class="mdi mdi-printer"></i> Cetak</a>
                                        <?php else: ?>
                                        -
                                        <?php endif; ?>
                                    </td>
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.user', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\xampp1\htdocs\hibah_upj\application\views/users/sponsorship/v_status_pencairan.blade.php ENDPATH**/ ?>